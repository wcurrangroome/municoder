...
- **`get_state_by_abbreviation(state_abbreviation)`**: Look up state
...
- **`get_client_products(client_id)`**: List all ordinance types
  (products) available for a jurisdiction (e.g., “Zoning”, “Code of
  Ordinances”)

### 3. Ordinance Navigation

- **`get_current_version(product_id)`**: Get the most current version of
  an ordinance
- **`get_version_history(product_id)`**: List all historical versions of
  an ordinance
- **`get_codes_toc(job_id, product_id)`**: Get table of contents for an
  ordinance
- **`get_section_ancestors(job_id, node_id, product_id)`**: Get ancestor
  nodes for a specific section
- **`get_section_children(job_id, node_id, product_id)`**: Get child
  nodes for a specific section

### 4. Content Extraction

- **`get_section_text(node_id, product_id)`**: Extract full text and
...
- **`get_ordinance_ancestors(product_id, node_id)`**: Navigate ordinance
  hierarchy
- **`get_ordinance_children(product_id, node_id)`**: Get children of
  ordinance nodes
...
Core dependencies: `dplyr`, `httr2`, `janitor`, `magrittr`, `purrr`,
...
- **Package structure (RECENTLY REFACTORED)**: The package has been
  modularized into 10 separate R files:
